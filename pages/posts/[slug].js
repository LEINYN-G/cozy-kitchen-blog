...
export default function PostPage({ frontmatter, content, theme, setTheme }) {
  ...
  return (
    ...
    >
      <style jsx global>{`
        /* 🟢 FIXED: PRO-LEVEL COMPACT IMAGE BOUNDS FOR PASTA & ALL BLOG POSTS */
        .markdown-content img {
          ...
          border-radius: 8px;
          margin: 2rem auto;
          box-shadow: 0 8px 24px rgba(0, 0, 0, 0.4), 0 0 1px rgba(255, 255, 255, 0.1); /* Subtle cyber outline glow */
          border: 1px solid rgba(255, 255, 255, 0.05);
          transition: transform 0.3s ease;
        }

        .markdown-content img:hover {
          transform: scale(1.02);              /* Clean dynamic focus pop on hover */
          border-color: #a4ff06;
        }
          ...
          <ReactMarkdown
            ...
            components={{
              ...
              h3({ children }) {
                ...
                if (
                  productName.toLowerCase().includes('instruction') ||
                  productName.toLowerCase().includes('ingredient')
                ) {
                  return (
                    <h3
                      style={{
                ...
                let productImage = '/images/default-thumb.jpg';

                // Fetch dynamic local images link form frontmatter
                if (productName.toLowerCase().includes('almond'))
                  productImage =
                    frontmatter.almond_img || productImage;

                if (productName.toLowerCase().includes('lavender'))
                  productImage =
                    frontmatter.lavender_img || productImage;

                if (productName.toLowerCase().includes('tea'))
                  productImage =
                    frontmatter.teatree_img || productImage;

                if (productName.toLowerCase().includes('beans'))
                  productImage =
                    frontmatter.beans_img || productImage;

                if (productName.toLowerCase().includes('oats'))
                  productImage =
                    frontmatter.oats_img || productImage;
              ...
              // 🚀 2. IMAGE-RESPONSIVE PARAGRAPH FILTER
              // (Prevents stretched recipe images)
              p({ children }) {
                const childrenArray = React.Children.toArray(children);

                const hasAffiliateNode = childrenArray.some(
                  (child) =>
                    typeof child === 'string' && child.includes('amzn.to')
                );
                ...
                const hasImageNode = childrenArray.some(
                  child =>
                    child &&
                    (child.type === 'img' ||
              ...
              a({ href, children }) {
                ...
                const isAmazonNode =
                  ...
                  linkText.includes('Amazon');

                if (isAmazonNode) {
                  const targetUrl = linkText.startsWith('http')
                    ? linkText
                    : href;
                  ...
                  return (
                    <div style={{ marginTop: '1rem' }}>
                      <a
                        ...
                        className="cyber-purdchase-trigger"
                      >
            ...
            >
              {frontmatter.products.map((item, index) => (
                ...
                >
                  ...
                  {/* 🚀 1. INJECTED FIXED NEXT.JS IMAGE LAYER WITH INTELLIGENT AUTO-FALLBACKS */}
                  <div style={{ position: 'relative', width: '100%', height: '120px', borderRadius: '6px', overflow: 'hidden', marginBottom: '0.8rem', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
                    <Image
                      src={
                        item.image || 
                        (item.name?.toLowerCase().includes('oil') 
                          ? 'https://unsplash.com' // HD Olive Oil Image
                          : item.name?.toLowerCase().includes('cutlery') || item.name?.toLowerCase().includes('set')
                          ? 'https://unsplash.com' // Sleek Cutlery Image
                          : '/images/default-thumb.jpg') // Fallback default image
                      }
